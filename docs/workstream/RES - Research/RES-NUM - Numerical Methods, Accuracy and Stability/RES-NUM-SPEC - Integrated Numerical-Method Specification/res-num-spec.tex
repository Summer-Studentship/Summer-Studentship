...
\begin{table}[htbp]
  ...
  \begin{tabular}{
    ...
    selected
    &
    Polygonal finite-volume update with triangular practical meshes,
    default first-order hydrodynamic reconstruction, opt-in
    limited-linear reconstruction using weighted least-squares gradients
    and Barth--Jespersen limiting, hydrostatic reconstruction with
    first-order bed states, Rusanov local Lax--Friedrichs flux,
    wet/dry positivity controls, SSPRK(3,3) transport, Manning friction,
...
The accepted mathematical model remains the nonlinear shallow-water
system with conserved state
$\mathbf{U}=[h,q_x,q_y]^{\mathsf{T}}$, bed-slope physics, Manning
friction, passive coseismic free-surface transfer, wetting and drying,
accepted artificial-boundary roles, and one-way Regional2D-to-Local3D
coupling variables. The reconstruction choice below is a numerical
discretisation choice only; changing from first-order face states to the
limited-linear option does not change this mathematical model.

The default and historical implementation uses piecewise-constant
hydrodynamic face states. The opt-in C1A-R9 limited-linear candidate
...
Weighted least-squares gradients are selected on the face-neighbour
stencil. Internal faces contribute the neighbouring cell centroid and
cell value. Boundary faces contribute the boundary face centroid and the
pre-existing exterior boundary value when that value is finite. With
$\mathbf{d}_{is}=\mathbf{x}_s-\mathbf{x}_i$ and
$w_{is}=1/\max(\|\mathbf{d}_{is}\|^2,\epsilon_w)$:
...
\begin{align}
  ...
  \mathbf{M}_i
  &=
  \sum_{s\in\mathcal{S}(i)}
  w_{is}
  \mathbf{d}_{is}
  \mathbf{d}_{is}^{\mathsf{T}}
...
The limited-linear path applies a scalar Barth--Jespersen limiter. The
limiter preserves constants and bounds reconstructed face values by
local stencil extrema, while allowing the method to fall back locally
towards first order near extrema, steep gradients or wet/dry-sensitive
states. Cells with an underdetermined or ill-conditioned least-squares
system use a zero gradient.

The verified production flux is Rusanov local Lax--Friedrichs. Each
face flux is solved in normal/tangential coordinates:
...
In the verified C1A-R9 implementation, the bed values entering
\cref{eq:res-num-spec-regional-hydrostatic-face-bed} remain the
cell-local first-order bed states. This preserves the established
well-balanced lake-at-rest behaviour while the hydrodynamic face states
are reconstructed at limited-linear order.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{
    ...
    triangular practical meshes; first-order and limited-linear
    hydrodynamic reconstruction; weighted least-squares gradients;
    Barth--Jespersen limiter; hydrostatic reconstruction; Rusanov flux
    strategy; wetting/drying; dynamic bed treatment; SSPRK(3,3)
    transport with Manning;
...
The project has selected the regional finite-volume family and
established two verified Regional2D numerical reconstruction paths for
the same NLSWE mathematical model: the default first-order path and the
opt-in limited-linear path. The latter is verified on controlled smooth
benchmarks but still requires event-level Tōhoku--Kamaishi convergence
assessment. The project has also selected the local three-dimensional
two-phase finite-volume solver family. It has not completed structural,
dimensional-coupling or FSI numerical-method selection. The numerical
architecture shall remain modular while implementation and validation
work proceeds.
...
\begin{itemize}
  \item final regional dry-depth tolerance and velocity regularisation
    constants;
  \item event-level adequacy of the verified Rusanov plus limited-linear
    Regional2D discretisation, with alternative flux families deferred;
