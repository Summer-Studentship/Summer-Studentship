...
\begin{itemize}
  ...
  \item default first-order hydrodynamic reconstruction;
  \item opt-in limited-linear hydrodynamic reconstruction;
  \item weighted least-squares gradients for the limited-linear path;
  \item Barth--Jespersen limiting for the limited-linear path;
  \item well-balanced hydrostatic reconstruction;
  \item positivity-preserving wet/dry treatment;
  \item Rusanov local Lax--Friedrichs flux in the verified production
    path;
...
\begin{itemize}
  ...
  \item event-level adequacy of the verified Rusanov plus
    limited-linear Regional2D discretisation;
...
\begin{itemize}
  ...
  \item `RES-NUM-FLX` shall document the verified first-order and
    limited-linear Rusanov paths and defer alternative flux families
    unless later convergence evidence requires them;
