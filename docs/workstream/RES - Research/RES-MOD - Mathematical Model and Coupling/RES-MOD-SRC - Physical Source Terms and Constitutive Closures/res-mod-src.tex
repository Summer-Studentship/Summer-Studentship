...
Manning bottom friction, instantaneous passive coseismic free-surface
transfer at initialisation, and optional sponge damping near artificial
open boundaries. Coriolis and lateral eddy viscosity are not part of the
baseline regional model; they remain optional sensitivity terms if the
selected domain scale or validation evidence requires them.
...
accepted earthquake contribution is applied as an instantaneous passive
free-surface initial condition,
$\eta(x,y,0^+)=\eta_0(x,y)+\Delta z_b(x,y)$; no additional
depth-equation mass source is added in the conserved-depth formulation.
Finite-rise-time rupture or dynamic moving-bed forcing is retained as a
future source-fidelity extension, not the current production baseline.
...
\begin{itemize}
  ...
  \item instantaneous passive coseismic transfer to the initial free
    surface, $\eta(x,y,0^+)=\eta_0(x,y)+\Delta z_b(x,y)$, with no
    separate depth-equation mass source;
...
\begin{itemize}
  ...
  \item final fault-source dataset and any future rupture-time
    functions for a dynamic moving-bed source-fidelity extension;
  \item lake-at-rest and passive earthquake-transfer verification
    evidence, plus any future moving-bottom verification evidence if a
    dynamic source extension is activated;
