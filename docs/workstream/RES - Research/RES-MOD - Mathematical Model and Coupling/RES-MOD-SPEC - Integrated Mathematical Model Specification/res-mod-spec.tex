...
open-boundary sponge damping. The accepted earthquake baseline is
instantaneous passive coseismic transfer to the initial free surface,
\cref{eq:res-mod-eqs-regional-passive-earthquake-transfer}. Since $h$
is the conserved depth variable, no separate earthquake mass source is
added to the depth equation. Finite-rise-time rupture or dynamic
moving-bed forcing is retained as a future source-fidelity extension,
...
\begin{quote}
  ...
  Manning sources, instantaneous passive coseismic transfer to the
  initial free surface, open/radiation boundaries, accepted
  characteristic/open-ocean damped treatment with sponge damping where
  required, and wetting--drying handled by the numerical method
\end{quote}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{
    ...
    convention; source-term catalogue; passive coseismic free-surface
    transfer baseline; open, reflective and sponge boundary roles with
    accepted characteristic/open-ocean damped numerical realisation
    &
