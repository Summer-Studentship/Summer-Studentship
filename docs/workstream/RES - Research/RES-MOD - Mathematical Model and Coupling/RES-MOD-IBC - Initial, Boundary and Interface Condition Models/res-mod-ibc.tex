...
\begin{table}[htbp]
  ...
  \begin{tabular}{
    ...
    &
    Selected as a boundary role; current numerical realisation uses the
    accepted characteristic/open-ocean damped treatment, while
    alternatives remain numerical sensitivities
    \\
    $\Gamma_{\mathrm{side}}$
    &
...
\begin{itemize}
  \item final QGIS/geospatial domain lock and artificial-boundary
    placement;
  \item alternative sponge-layer widths and damping scales as numerical
    sensitivity studies;
  ...
  \item local wall-function and roughness parameters for terrain and
    barrier surfaces;
  \item alternative local outlet and side-boundary absorption settings.
\end{itemize}
