...
\usepackage{booktabs}
\usepackage{balance}
% Keep hyperlinks active while preserving IEEE's print appearance.
\usepackage[hidelinks,unicode]{hyperref}

\hypersetup{
  pdftitle={A Coupled 2D NLSWE Regional-Propagation and 3D URANS-VOF Coastal-Impact Framework for Reconstructing the 2011 Tohoku Tsunami},
  pdfauthor={Rhyen Jivan},
  pdfsubject={One-way coupled regional tsunami propagation and local coastal-impact modelling},
  pdfkeywords={tsunami modelling, nonlinear shallow-water equations, URANS-VOF, multiscale coupling, 2011 Tohoku tsunami}
}
\urlstyle{same}
...
explicit evidence gates. Appendix~\ref{app:planning} preserves the original
poster workflow and project roadmap. Figure~\ref{fig:framework} presents the
resulting model architecture in the same visual language; current evidence
establishes implementation and verification closure, while observational
calibration and predictive validation remain future work.
...
\appendices
\section{Original Studentship Workflow}
\label{app:planning}

The original poster workflow is reproduced unchanged in
Fig.~\ref{fig:planning}. It records the planned progression from research and
event reconstruction through Regional2D propagation, one-way hybrid replay,
verification, validation and later defence-system extensions. The progress
marker reflects the project state at the time of poster production.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{planning-workflow-original.png}
  \caption{Original studentship planning and architecture workflow, retained
  from the project poster. The roadmap distinguishes implemented regional and
  hybrid stages from verification, validation and later defence-system work.}
  \label{fig:planning}
\end{figure*}
