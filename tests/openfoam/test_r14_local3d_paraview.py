...
class R14Local3DParaviewTests(unittest.TestCase):
    ...
    def test_discover_case_reports_times_fields_and_vtk(self):
        ...
        (self.tmp / "0/U").write_text("field\n", encoding="utf-8")
        (self.tmp / "0/p_rgh").write_text("field\n", encoding="utf-8")
        (self.tmp / "1").mkdir()
        (self.tmp / "1/U").write_text("field\n", encoding="utf-8")
        (self.tmp / "VTK").mkdir()
        (self.tmp / "VTK/case_1.vtk").write_text("# vtk\n", encoding="utf-8")
        (self.tmp / "VTK/inlet").mkdir()
        (self.tmp / "VTK/inlet/inlet_1.vtk").write_text("# vtk\n", encoding="utf-8")
        ...
        self.assertEqual(discovered["alpha_field_name"], "alpha.water")
        self.assertEqual(discovered["velocity_field_name"], "U")
        self.assertEqual(discovered["pressure_field_name"], "p_rgh")
        self.assertEqual(discovered["patch_names"], ["inlet"])
        self.assertIn("overview", discovered["camera_presets"])
        self.assertIn("ocean", discovered["style_profiles"])
        self.assertEqual(discovered["free_surface_extraction"]["isovalue"], 0.5)
        self.assertTrue(discovered["has_case_foam"])
        self.assertEqual(len(discovered["vtk_files"]), 1)
        self.assertEqual(discovered["root_vtk_records"][0]["time_s"], 0.0)
